...
from .models import Layanan, RekomendasiTeknis, Intake, StatusRekomendasiTeknis, PelaksanaanKontruksiDanPengalihanAir
...
class RekomendasiTeknisAdmin(admin.ModelAdmin):
    ...
    def aksi(self, obj):
        change_url = reverse("admin:rekomtek_rekomendasiteknis_change", args=[obj.id])
        delete_url = reverse("admin:rekomtek_rekomendasiteknis_delete", args=[obj.id])
        return format_html(
...
# ===== Admin RekomendasiTeknis =====
@admin.register(PelaksanaanKontruksiDanPengalihanAir)
class PelaksanaanKontruksiDanPengalihanAirAdmin(admin.ModelAdmin):
    list_display = ['layanan', 'nama_pemohon', 'tanggal_permohonan', 'aksi']
    list_filter = ['layanan', 'tanggal_permohonan']
    search_fields = ['nama_pemohon']
    actions = [export_to_word, export_to_pdf]
    readonly_fields = ['user']  # user hanya bisa dibaca
    ...
    def aksi(self, obj):
        change_url = reverse("admin:rekomtek_pelaksanaankontruksidanpengalihanair_change", args=[obj.id])
        delete_url = reverse("admin:rekomtek_pelaksanaankontruksidanpengalihanair_delete", args=[obj.id])
        return format_html(
            '<div style="display: flex; gap: 5px;">'
            '<a class="btn btn-warning btn-sm" href="{}">Edit</a>'
            '<a class="btn btn-danger btn-sm" href="{}">Hapus</a>'
...
class StatusRekomendasiTeknisAdmin(admin.ModelAdmin):
    list_display = [
        'rekomtek',
        'pelaksanaan_link',  # ganti pelaksanaan dengan versi link
        'status',
    ...
    list_filter = ['rekomtek', 'pelaksanaan']

    def pelaksanaan_link(self, obj):
        if obj.pelaksanaan:
            url = reverse("admin:rekomtek_pelaksanaankontruksidanpengalihanair_change", args=[obj.pelaksanaan.id])
            return format_html('<a href="{}">{}</a>', url, obj.pelaksanaan)
        return "-"
    pelaksanaan_link.short_description = "Pelaksanaan"

    def aksi(self, obj):
        change_url = reverse("admin:rekomtek_statusrekomendasiteknis_change", args=[obj.id])
        delete_url = reverse("admin:rekomtek_statusrekomendasiteknis_delete", args=[obj.id])
        return format_html(
            '<div style="display: flex; gap: 5px;">'
            '<a class="btn btn-warning btn-sm" href="{}">Edit</a>'
            '<a class="btn btn-danger btn-sm" href="{}">Hapus</a>'
